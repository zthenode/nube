...
<xsl:param name="hyde_form_title" select="'Create SQL'"/>
<xsl:param name="hyde_form_submit" select="'Submit'"/>
<xsl:param name="hyde_form_method" select="'post'"/>
<xsl:param name="hyde_form_action" select="'../t/sql.t'"/>
...
<xsl:template name="form_fields">
    <xsl:call-template name="form_creation_parameters">
        <!--xsl:with-param name="document_file" select="'../xml/hyde/db-schema.xml'"/>
        <xsl:with-param name="template_file" select="'../xml/hyde/db-schema-to-sql.xsl'"/>
        <xsl:with-param name="content_type" select="'text/plain'"/-->
    </xsl:call-template>
    <xsl:call-template name="section_row">
        <xsl:with-param name="text" select="'SQL Parameters'"/>
    </xsl:call-template>
